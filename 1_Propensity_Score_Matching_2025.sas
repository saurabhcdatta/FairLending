/* ============================================================
   *** 2025 REPLICATION - PRE-RUN VERIFICATION CHECKLIST ***
   ...
   (2024_NEW->2025_NEW, hmda24->hmda25) PLUS one substantive
   addition for 2025: a sixth outcome (Orig_Charges_Prc_LA,
   origination charges as % of loan amount) has been added so
   that the 2025 results can be compared row-by-row with the
   2024 four-method summary, which reported origination charges
   as a separate outcome. The methodology for that outcome
   mirrors how Loan_Cost_Prc_LA is treated (% of loan amount,
   GLM and IPW-DR adjusted, with Loan_Cost_Prc_LA dropped from
   controls when Orig_Charges_Prc_LA is the outcome because
   the two are in a subset relationship).
   ...
   2. The following variables exist in hmda25.eslfcu with the
      ...
      ALSO required (used for the new origination-charges
      outcome added in 2025):

      Orig charges: orig_charges (character, dollar amount
                    stored as text)
   ...
   5. The all_in_cost composite is constructed inside this
      ...
      variable named all_in_cost needs to exist. The new
      Orig_Charges_Prc_LA outcome is ALSO constructed inline
      from the character variable orig_charges divided by
      loan_Amt; non-numeric origination-charge values become
   ...
   The output will now contain SIX outcomes per group instead
   of five. The new outcome appears as 'Orig_Charges_Prc_LA' in
   the GLM_Results and IPW_DR sheets and in the Full_Detail
   crosstab. Flagged Adverse Findings will pick up any rows
   where the new outcome's p-value is below 0.05.
   ============================================================ */
...
/* ============================================================
   ...
   Outcomes: Rate, Points, Credits, Cost%, Orig Chg%, All-In Cost
...
data hmda_race;
  ...
  /* Convert origination charges from character to numeric, scaled by loan
     amount to match the Loan_Cost_Prc_LA convention. Defensive parse so
     non-numeric codes (e.g. "NA", "Exempt") become missing rather than
     erroring. Note: origination charges is a SUBSET of total closing costs
     (Loan_Cost_Prc_LA), so Orig_Charges_Prc_LA is NOT included in the
     all_in_cost composite below (would double-count). */
  orig_charges_num = input(orig_charges, ?? best32.);
  if not missing(orig_charges_num) and not missing(loan_Amt) and loan_Amt > 0
    then Orig_Charges_Prc_LA = orig_charges_num / loan_Amt;
    else Orig_Charges_Prc_LA = .;

  /* Composite "all-in cost" - unchanged. Loan_Cost_Prc_LA already includes
     origination charges, so adding Orig_Charges_Prc_LA here would
     double-count that fee. */
...
proc means data=hmda_race n mean stddev min max;
  var numeric_rate Discount_Points_LA Lender_Credits_LA
      Loan_Cost_Prc_LA Orig_Charges_Prc_LA all_in_cost;
...
%macro fair_lending_psm(
  ...
  /* ----------------------------------------------------------
     2F: LOOP OVER SIX PRICING OUTCOMES
     CRITICAL: Outcome variable is excluded from controls.
     ALSO: Loan_Cost_Prc_LA is excluded as a control when the
     outcome is Orig_Charges_Prc_LA, because origination charges
     are a subset of total loan costs (subset relationship would
     create endogeneity if both were on the model at once).
     ---------------------------------------------------------- */
  %let outcomes = numeric_rate Discount_Points_LA
                  Lender_Credits_LA Loan_Cost_Prc_LA
                  Orig_Charges_Prc_LA all_in_cost;
  %let i = 1;
  ...
  %do %while (%scan(&outcomes, &i) ne );
    ...
    %if &var ne Loan_Cost_Prc_LA and &var ne Orig_Charges_Prc_LA %then
